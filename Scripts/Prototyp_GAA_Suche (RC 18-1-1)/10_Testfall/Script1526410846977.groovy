...
ArrayList<WebElement> wes1 = WebUiCommonHelper.findWebElements(findTestObject('gaa_map'), 5)
...
if (filial_maps_count != filial_count) {
    ...
    throw new StepFailedException((('Anzahl Maps ' + filial_maps_count) + ' ist NICHT gleich Anzahl GAA ') + filial_count)
} else if (filial_accordeon_count != filial_count) {
    WebUI.comment((('Anzahl Accordeons ' + filial_accordeon_count) + ' ist NICHT gleich Anzahl GAA ') + filial_count)

    WebUI.closeBrowser()

    throw new StepFailedException((('Anzahl Accordeons ' + filial_accordeon_count) + ' ist NICHT gleich Anzahl GAA ') + 
    filial_count)
}
